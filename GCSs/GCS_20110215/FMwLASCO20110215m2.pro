...
;Global Paths
SAVE_FILE=GCSD_PATH+'/GCS_20110215/m2.sav'
secchipath=DATA_PATH+'/stereo/secchi/L0'
lascopath=DATA_PATH+'/soho/lasco/level_1/c2'
sdopath=DATA_PATH+'/sdo'
PLAY_MOVIE=[0,0,0] ; set to 1 to play the corresponding movie [euvia, euvib, aia]
...
if PLAY_MOVIE[0] then begin
 euvia_mov=DATA_PATH+'/stereo/secchi/L0/a/img/euvi/195/'+SDATE0+'/preped/'+SDATE0+'_EUVIA195_rdiff.mvi'
 scc_playmovie, euvia_mov
endif
if PLAY_MOVIE[1] then begin
 euvia_mov=DATA_PATH+'/stereo/secchi/L0/b/img/euvi/195/'+SDATE0+'/preped/'+SDATE0+'_EUVIB195_rdiff.mvi'
 scc_playmovie, euvia_mov
endif
...
peuvia0=secchipath+'/a/img/euvi/195/'+SDATE0+'/preped/'+SDATE0+'_'+STIME0+'_14euA.fts'
peuvia =secchipath+'/a/img/euvi/195/'+SDATE+'/preped/'+SDATE+'_'+STIME+'_14euA.fts'
if ~file_test(peuvia0) then begin 
  bla=secchipath+'/a/img/euvi/195/'+SDATE0+'/'+SDATE0+'_'+STIME0+'_n4euA.fts'
...
if ~file_test(peuvia) then begin 
  bla =secchipath+'/a/img/euvi/195/'+SDATE+'/'+SDATE+'_'+STIME+'_n4euA.fts' 
...
peuvib0=secchipath+'/b/img/euvi/195/'+SDATE0+'/preped/'+SDATE0+'_'+STIME0+'_14euB.fts'
peuvib =secchipath+'/b/img/euvi/195/'+SDATE+'/preped/'+SDATE+'_'+STIME+'_14euB.fts'
if ~file_test(peuvib0) then begin 
  bla=secchipath+'/b/img/euvi/195/'+SDATE0+'/'+SDATE0+'_'+STIME0+'_n4euB.fts'
...
if ~file_test(peuvib) then begin 
  bla =secchipath+'/b/img/euvi/195/'+SDATE+'/'+SDATE+'_'+STIME+'_n4euB.fts' 
...
;diff images scaling
euvia=smooth(bytscl(euvia,min=mean(euvia)+RNG[0]*stddev(euvia), max=mean(euvia)+RNG[1]*stddev(euvia)),3)
euvib=smooth(bytscl(euvib,min=mean(euvib)+RNG[2]*stddev(euvib), max=mean(euvib)+RNG[3]*stddev(euvib)),3)
aia=smooth(bytscl(aia,min=mean(aia)+RNG[4]*stddev(aia), max=mean(aia)+RNG[5]*stddev(aia)),3)
