...
;Global Paths
SAVE_FILE=GCSD_PATH+'/GCS_20130608/m1.sav'
secchipath=DATA_PATH+'/stereo/secchi/L0'
lascopath=DATA_PATH+'/soho/lasco/level_1/c2'
sdopath=DATA_PATH+'/sdo'

PLAY_MOVIE=0.*[1,1,1] ; set to 1 to play the corresponding movie [euvia, euvib, aia]
...
if PLAY_MOVIE[0] then begin
 euvia_mov=DATA_PATH+'/stereo/secchi/L0/a/img/euvi/'+SDATEDIR+'/preped/'+SDATEDIR+'_EUVIA195_rdiff.mvi'
 scc_playmovie, euvia_mov
endif
if PLAY_MOVIE[1] then begin
 euvia_mov=DATA_PATH+'/stereo/secchi/L0/b/img/euvi/'+SDATEDIR+'/preped/'+SDATEDIR+'_EUVIB195_rdiff.mvi'
 scc_playmovie, euvia_mov
endif
if PLAY_MOVIE[2] then begin
 euvia_mov=DATA_PATH+'/sdo/aia/L1/193/'+SDATEDIR+'/preped/'+SDATEDIR+'_AIA193_rdiff.mvi'
 scc_playmovie, euvia_mov
endif

;SECCHIA/EUVI (to be used as CORA)
peuvia0=secchipath+'/a/img/euvi/'+SDATEDIR+'/preped/'+SDATE0+'_'+STIME0+'_14euA.fts'
peuvia =secchipath+'/a/img/euvi/'+SDATEDIR+'/preped/'+SDATE+'_'+STIME+'_14euA.fts'
if ~file_test(peuvia0) then begin 
  bla=secchipath+'/a/img/euvi/'+SDATEDIR+'/'+SDATE0+'_'+STIME0+'_n4euA.fts'
...
if ~file_test(peuvia) then begin 
  bla =secchipath+'/a/img/euvi/'+SDATEDIR+'/'+SDATE+'_'+STIME+'_n4euA.fts' 
...
peuvib0=secchipath+'/b/img/euvi/'+SDATEDIR+'/preped/'+SDATE0+'_'+STIME0+'_14euB.fts'
peuvib =secchipath+'/b/img/euvi/'+SDATEDIR+'/preped/'+SDATE+'_'+STIME+'_14euB.fts'
if ~file_test(peuvib0) then begin 
  bla=secchipath+'/b/img/euvi/'+SDATEDIR+'/'+SDATE0+'_'+STIME0+'_n4euB.fts'
...
if ~file_test(peuvib) then begin 
  bla =secchipath+'/b/img/euvi/'+SDATEDIR+'/'+SDATE+'_'+STIME+'_n4euB.fts' 
...
oaia0=sdopath+'/aia/L1/193/'+strjoin(SDATEDIR)+'/preped/AIA'+strjoin(DATE0)+'_'+strjoin(TIME0)+'_0193.fits'
oaia =sdopath+'/aia/L1/193/'+strjoin(SDATEDIR)+'/preped/AIA'+strjoin(DATE)+'_'+strjoin(TIME)+'_0193.fits'
if ~file_test(oaia0) then begin 
  paia0=sdopath+'/aia/L1/193/'+strjoin(SDATEDIR)+'/aia.lev1.193A_'+DATE0[0]+'-'+DATE0[1]+'-'+$ 
...
if ~file_test(oaia) then begin
  paia =sdopath+'/aia/L1/193/'+strjoin(SDATEDIR)+'/aia.lev1.193A_'+DATE[0]+'-'+DATE[1]+'-'+$ 
